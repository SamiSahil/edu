...
export function Select({ label, helperText, error, className, id, children, ...props }) {
  ...
  return (
    <label className="block space-y-2 text-sm text-zinc-200" htmlFor={inputId}>
      ...
      <select
        id={inputId}
        className={cn(
          'h-11 w-full rounded-2xl border bg-zinc-950 px-4 text-sm text-white outline-none transition focus:border-emerald-400/70 focus:ring-2 focus:ring-emerald-400/15 disabled:cursor-not-allowed disabled:opacity-60',
